...
DescriptionStart a buildUsage`docker buildx dap build [OPTIONS] PATH | URL | -`
...
NameTypeDefaultDescription`dockerfile``string``Dockerfile`Name of the Dockerfile`contextPath``string``.`Set the context path for the build (normally the first positional argument)`target``string`Set the target build stage to build`stopOnEntry``boolean``false`Stop on the first instruction
