...
DescriptionShow details of an image in the registryUsage`docker buildx imagetools inspect [OPTIONS] NAME`
...
OptionDefaultDescription[`--format`](#format)`` `{{.Manifest}}` ``Format the output using the given Go template[`--raw`](#raw)Show original, unformatted JSON manifest
