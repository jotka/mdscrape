...
DescriptionRemove unused imagesUsage`docker image prune [OPTIONS]`
...
OptionDefaultDescription`-a, --all`Remove all unused images, not just dangling ones[`--filter`](#filter)Provide filter values (e.g. `until=<timestamp>`)`-f, --force`Do not prompt for confirmation
