...
OptionDefaultDescription[`-a, --all-tags`](#all-tags)Push all tags of an image to the repository`--disable-content-trust``true`Skip image signing`--platform`API 1.46+ Push a platform-specific manifest as a single-platform image to the registry.  
