...
OptionDefaultDescription[`-a, --all-tags`](#all-tags)Download all tagged images in the repository`--disable-content-trust``true`Skip image verification`--platform`API 1.32+ Set platform if server is multi-platform capable`-q, --quiet`Suppress verbose output
