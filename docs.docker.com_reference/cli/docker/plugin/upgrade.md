...
DescriptionUpgrade an existing pluginUsage`docker plugin upgrade [OPTIONS] PLUGIN [REMOTE]`
...
OptionDefaultDescription`--disable-content-trust``true`Skip image verification`--grant-all-permissions`Grant all permissions necessary to run the plugin`--skip-remote-check`Do not check if specified remote plugin matches existing plugin image
