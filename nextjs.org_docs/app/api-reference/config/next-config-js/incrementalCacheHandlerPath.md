...
ParameterTypeDescription`tag``string` or `string[]`The cache tags to revalidate.
...
VersionChanges`v14.1.0`Renamed to `cacheHandler` and became stable.`v13.4.0``incrementalCacheHandlerPath` support for `revalidateTag`.`v13.4.0``incrementalCacheHandlerPath` support for standalone output.`v12.2.0`Experimental `incrementalCacheHandlerPath` added.
