...
VersionChanges`v16.0.0``id` is now a promise that resolves to a number.`v14.2.0`Add localizations support.`v13.4.14`Add `changeFrequency` and `priority` attributes to sitemaps.`v13.3.0``sitemap` introduced.
