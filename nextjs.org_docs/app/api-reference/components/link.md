...
PropExampleTypeRequired[`href`](#href-required)`href="/dashboard"`String or ObjectYes[`replace`](#replace)`replace={false}`Boolean-[`scroll`](#scroll)`scroll={false}`Boolean-[`prefetch`](#prefetch)`prefetch={false}`Boolean or null-[`onNavigate`](#onnavigate)`onNavigate={(e) => {}}`Function-
